...
function roleBadge(role: string | null) {
  if (!role) return { label: "미설정", bg: "rgba(255,255,255,0.05)", color: "var(--color-text-muted)" };
  const map: Record<string, { label: string; bg: string; color: string }> = {
    admin:        { label: "ADMIN",        bg: "rgba(167,139,250,0.2)", color: "#a78bfa" },
    judge:        { label: "JUDGE",        bg: "rgba(99,102,241,0.2)",  color: "#818cf8" },
    photographer: { label: "PHOTO",        bg: "rgba(34,197,94,0.15)",  color: "#4ade80" },
    participant:  { label: "PARTICIPANT",  bg: "rgba(255,255,255,0.07)",color: "var(--color-text-secondary)" },
  };
  return map[role] ?? { label: role.toUpperCase(), bg: "rgba(255,255,255,0.07)", color: "var(--color-text-secondary)" };
}
...
export default function ImffAdminPage() {
  const [stats, setStats] = useState<ImffStats | null>(null);
  const [users, setUsers] = useState<ImffUser[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/imff");
      ...
      console.error("IMFF admin fetch error:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      {loading && !stats ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="glass-card p-5 rounded-2xl group hover:scale-[1.02] transition-transform duration-200">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "rgba(251,191,36,0.2)" }}>
      ...
      {/* ── API 연동 정보 배너 ── */}
      <div className="glass-card rounded-2xl p-5 border" style={{ borderColor: "rgba(99,102,241,0.2)" }}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid gap-1.5">
              {[
                ...
                <div key={ep.path} className="flex items-center gap-3 text-xs">
                  <span className="shrink-0 w-12 text-center py-0.5 rounded font-bold font-mono"
                    style={{
                      backgroundColor: ep.method === "POST" ? "rgba(99,102,241,0.2)" : "rgba(34,197,94,0.15)",
                      color: ep.method === "POST" ? "#818cf8" : "#4ade80",
                    }}>
                    {ep.method}
                  </span>
            ...
            <p className="text-xs mt-3" style={{ color: "var(--color-text-muted)" }}>
              모든 요청에 <code className="text-indigo-400">x-api-secret</code> 헤더가 필요합니다.
              IMFF localUserId는 Firebase UID(문자열)를 사용합니다.
            </p>
          </div>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center" style={{ color: "var(--color-text-muted)" }}>
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p>{search ? "검색 결과가 없습니다." : "아직 IMFF 앱에서 연동된 회원이 없습니다.\nIMFF 앱에서 로그인하면 자동으로 여기에 표시됩니다."}</p>
                    </div>
                  </td>
                </tr>
              ) : (
